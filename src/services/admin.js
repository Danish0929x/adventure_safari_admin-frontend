...
const API_BASE_URL =
  "https://adventure-safari-admin-backend2-1.onrender.com/api";
...
export const adminService = {
  ...
  getAllBookings: async () => {
    return await makeAuthenticatedRequest(
      `${API_BASE_URL}/admin/get-all-bookings`
    );
  },

  getBookingById: async (id) => {
    return await makeAuthenticatedRequest(
      `${API_BASE_URL}/admin/get-booking/${id}`
    );
  },
  ...
  toggleTripStatus: async (id) => {
    return await makeAuthenticatedRequest(
      `${API_BASE_URL}/trips/${id}/toggle-status`,
      {
        method: "PATCH",
      }
    );
  ...
  createBooking: async (bookingData) => {
    return await makeAuthenticatedRequest(
      `${API_BASE_URL}/admin/create-booking`,
      {
        method: "POST",
...
export const getBookingById = (id) => adminService.getBookingById(id);
...
export const updateTrip = (id, tripData) =>
  adminService.updateTrip(id, tripData);
